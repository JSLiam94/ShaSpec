...
def compute_hd95_single(pred, label, batch_size=1):
    ...
    pred_points = np.argwhere(pred > 0)  # 修改条件为大于0
    label_points = np.argwhere(label > 0)  # 修改条件为大于0
...
def compute_hd95(preds, labels, batch_size=1, num_threads=2):
    """
...
def main():
    ...
    testloader = data.DataLoader(
        ...
        batch_size=1, shuffle=False, pin_memory=True, num_workers=8)
    ...
    for index, batch in enumerate(testloader):
        ...
        seg_pred = np.where(np.logical_or(seg_pred_WT == 1, seg_pred_WT == 2 ,seg_pred_WT ==3), 2, seg_pred)  # 修改条件为1,2,3
        seg_pred = np.where(np.logical_or(seg_pred_TC == 1, seg_pred_TC == 3), 1, seg_pred)
        seg_pred = np.where(seg_pred_ET == 4, 4, seg_pred)  # 修改条件为等于4
        ...
        seg_gt = np.where(np.logical_or(seg_gt_WT == 1, seg_gt_WT == 2, seg_gt_WT==3), 2, seg_gt)  # 修改条件为1,2,3
        seg_gt = np.where(np.logical_or(seg_gt_TC == 1, seg_gt_TC == 3), 1, seg_gt)
        seg_gt = np.where(seg_gt_ET == 4, 4, seg_gt)  # 修改条件为等于4
